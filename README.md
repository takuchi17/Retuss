...
ユーザにメイン(UML)ウィンドウとソースコードウィンドウを表示する機能を提供します。
...
`src\main\java\io\github\morichan\retuss\controller` ディレクトリ下のファイルに該当します。


## 描画部

PlantUMLコードから、クラス図とシーケンス図を描画部に表示するための処理を提供します。

`src\main\java\io\github\morichan\retuss\drawer` ディレクトリ下のファイルに該当します。
...
`src/main/java/io/github/morichan/retuss/translator` ディレクトリ下のファイルに該当します。
...
基本的に `src\main\java\io\github\morichan\retuss\translator\cpp` ファイルです。

### Java

基本的に `src\main\java\io\github\morichan\retuss\translator\java` ファイルです。
...
基本的に `src\main\java\io\github\morichan\retuss\model` ファイルです。
...
Java、C++共にテストがほとんどありません。
過去の構造においてのテストも残存していて、```gradle build```コマンドが通りません。
...
### C++とJavaで共通のインタフェースを実装したり、クラスを継承するようにする

実装期間の都合上、それぞれが独立した設計になっており、統一性がほとんどありません。
ディレクトリ構成もそうですが、クラス設計から行う必要があります。


# 注意点

* 現在、テストの整備ができていないため、テストコードのコンパイルができません。そのため、gradle buildコマンドではなく、gradle assembleコマンドで、プロダクトのみのビルドを行うようにしてください。
